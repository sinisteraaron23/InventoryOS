...
import { Printer, X, Check, Box, Package, Sliders } from 'lucide-react';
...
export const BarcodePrintModal: React.FC<BarcodePrintModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-xs p-4 overflow-y-auto print:p-0 print:bg-white print:static print:inset-auto">
      ...
      <div className="relative w-full max-w-2xl bg-white rounded-3xl shadow-2xl border border-slate-200 overflow-hidden flex flex-col max-h-[92vh] print:max-h-none print:shadow-none print:border-none print:w-full print:max-w-none">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              >
                ...
                <option value="compact">Compact Cable/Dev Tag (2.2" × 1.0")</option>
                <option value="standard">Standard Asset Tag (3.5" × 1.5")</option>
                <option value="tote">Large Bin Placard (4.0" × 2.5")</option>
              </select>
            </div>
          </div>
        </div>
        ...
        <div className="p-6 overflow-y-auto flex-1 bg-slate-100/70 dark:bg-slate-950 print:bg-white print:p-0">
          ...
          {(labelSize === 'ptouch_12mm' || labelSize === 'ptouch_9mm') && (
            <div className="mb-3 px-3.5 py-2.5 bg-indigo-50/80 dark:bg-indigo-950/60 border border-indigo-200/80 dark:border-indigo-800/70 rounded-2xl flex items-start gap-2.5 text-xs text-indigo-950 dark:text-indigo-200 print:hidden">
              <span className="text-base leading-none mt-0.5">🏷️</span>
              <div className="text-[11px] leading-normal">
                <span className="font-bold">Brother P-Touch Cube PT-P300BT Preset:</span> Sized precisely for <strong>{labelSize === 'ptouch_12mm' ? '12mm (0.47")' : '9mm (0.35")'} TZe thermal tape</strong>. In your print prompt, select your Brother Cube, set Margins to <strong>None</strong> and Scale to <strong>100%</strong>.
              </div>
            </div>
          )}
          ...
          <div className="print-surface flex flex-wrap gap-4 items-start justify-center">
            ...
            {printTarget === 'item' && currentItem && (
              Array.from({ length: copies }).map((_, idx) => {
                ...
                return (
                  <div
                    key={idx}
                    className={`bg-white border-2 border-slate-900 rounded-lg p-3 text-slate-950 flex flex-col justify-between shadow-xs print:shadow-none print:m-1 ${
                      labelSize === 'compact'
                        ? 'w-[220px] min-h-[105px]'
                        : labelSize === 'tote'
                        ? 'w-[360px] min-h-[190px]'
                  ...
                  >
                    <div className="flex justify-between items-start border-b border-slate-900/30 pb-1.5 gap-2">
                      ...
                      {currentItem.protocol && currentItem.protocol !== 'None' && (
                        <span className="text-[9px] font-bold uppercase tracking-wider px-1.5 py-0.5 border border-slate-900 rounded bg-slate-100 text-slate-900 shrink-0">
                    ...
                    {/* Visual Barcode */}
                    <div className="my-1.5 py-0.5 bg-white flex justify-center">
                      <BarcodeRenderer
                        value={currentItem.barcode}
                        width={labelSize === 'compact' ? 1.2 : 1.6}
                        height={labelSize === 'compact' ? 32 : labelSize === 'tote' ? 52 : 40}
                        fontSize={11}
                        displayValue={true}
                        lineColor="#000000"
                      />
            ...
            {printTarget === 'box' && currentBox && (
              <div
                className={`bg-white border-3 border-slate-950 rounded-xl p-4 text-slate-950 flex flex-col justify-between shadow-xs print:shadow-none ${
                  labelSize === 'compact' ? 'w-[260px]' : 'w-[380px]'
                }`}
              >
        ...
        {/* Modal Action Bar */}
        <div className="print:hidden px-6 py-4 border-t border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/90 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <button
              onClick={onClose}
              className="px-4 py-2 border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700/60 rounded-xl text-xs font-semibold text-slate-700 dark:text-slate-200 transition-colors"
            >
              Cancel
            </button>
            <button
              onClick={handlePrint}
              className="px-5 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-semibold shadow-xs shadow-indigo-500/20 transition-colors flex items-center gap-2"
            >
